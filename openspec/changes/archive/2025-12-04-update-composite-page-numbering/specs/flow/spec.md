...
Orchestrator SHALL 使用新的复合页码 `<submoduleIndex>.<pageIndexTwoDigits>` 并在 `pageDesc` 追加 `[flowId/submoduleId/stepIndex]` 以增强可观测性。

#### Scenario: 复合页码解析
- WHEN 解析 Progress 或计算下一页
- THEN Orchestrator SHALL 使用 `encodeCompositePageNum(submoduleIndex, pageIndex)` 生成复合页码；
- AND 计数规则如下：
  - `submoduleIndex = stepIndex + 1`（Flow stepIndex 从 0 开始，submoduleIndex 从 1 开始）；
  - `pageIndex = modulePageNum`（modulePageNum 已约定从 `'1'` 开始，解析为数字后直接使用）；
  - 若 modulePageNum 为字符串，需先 `parseInt(modulePageNum, 10)` 再传入；
  - 零填充由 `encodeCompositePageNum` 内部处理，调用者无需手动填充；
- AND 示例映射：`{ stepIndex: 0, modulePageNum: '3' }` → `encodeCompositePageNum(1, 3)` → `"1.03"`；
- AND 生成的 MarkObject.pageDesc SHALL 附带 `[flowId/submoduleId/stepIndex]` 前缀（注意 stepIndex 保持原值 0，不做 +1）；
- AND 旧格式处理：
  - **禁止新生成**：`M<stepIndex>:<subPageNum>`/`0.*`/无零填充格式均不得新产生；
  - **允许读取兼容**：`parseCompositePageNum` MAY 接受旧格式用于 Progress 恢复；
- AND 复合页码 MUST 满足正则 `^[1-9]\d*\.\d{2}$`。
...
- THEN 提交构建的 `MarkObject` SHALL：
  ...
  - 使用 `encodeCompositePageNum(submoduleIndex, pageIndex)` 生成 pageNumber（`submoduleIndex = stepIndex + 1`，两位零填充，如 `1.01`、`1.10`），并在 `targetElement` 前缀中复用同一 pageNumber。

#### Scenario: 与复合页码的协同
- WHEN Flow 使用 Progress `{ stepIndex, modulePageNum }` 或等价信息恢复当前页
- THEN 子模块构建的 `MarkObject.pageNumber` SHALL 与该 Progress 一致；
- AND 转换规则：`encodeCompositePageNum(stepIndex + 1, parseInt(modulePageNum, 10))`；
- AND 示例：`{ stepIndex: 0, modulePageNum: '3' }` → `"1.03"`，`{ stepIndex: 1, modulePageNum: '10' }` → `"2.10"`；
- AND 禁止继续使用 `0.*` 或 `M*` 编码；
