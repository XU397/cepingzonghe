...
const Page06Experiment1: React.FC = () => {
  ...
  const questionTarget = `${targetPrefix}Q2_50cm风速区域`;
  ...
  // Initialize choice from answers
  useEffect(() => {
    const savedChoice = answers['experiment1Choice'];
    if (savedChoice && typeof savedChoice === 'string') {
  ...
  const handleStart = () => {
    ...
    setTimeout(() => {
      const wpSpeed = withPanelData[heightIndex];
      const npSpeed = withoutPanelData[heightIndex];

      setWithPanelSpeed(wpSpeed);
      setWithoutPanelSpeed(npSpeed);
      setIsRunning(false);

      // Update experiment state with collected data
      updateExperimentState({
        collectedData: {
          heightLevel: currentHeight,
          withPanelSpeed: wpSpeed,
          noPanelSpeed: npSpeed,
