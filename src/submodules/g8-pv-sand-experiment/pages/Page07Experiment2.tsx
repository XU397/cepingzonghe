...
import styles from '../styles/Page07_Experiment2.module.css';
...
const Page07Experiment2: React.FC = () => {
  ...
  const options = [
    '两区域风速均随高度增加而持续增大',
    '两区域风速均随高度增加而持续减小',
    '无板区风速随高度增加而增大;有板区风速随高度增加先增大后减小',
    '无板区风速随高度增加而增大;有板区风速随高度增加而减小'
  ];

  // Initialize from saved answers
  useEffect(() => {
    const savedAnalysis = answers['experiment2Analysis'];
    if (savedAnalysis && typeof savedAnalysis === 'string') {
      setSelectedOption(savedAnalysis);
    }
  }, []);
      ...
      <div className={styles.rightPanel}>
        ...
        <div className={styles.questionCard}>
          <p className={styles.questionText}>
            根据实验模拟，关于风速随高度变化的描述，正确的是？
          </p>
          <div className={styles.optionsContainer}>
            {options.map((option, index) => (
              <button
                key={index}
                className={`${styles.optionButton} ${selectedOption === option ? styles.optionSelected : ''}`}
