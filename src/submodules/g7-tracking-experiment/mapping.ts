...
const EXPERIMENT_MAX_PAGE = 12;
const PAGE_MAPPING_RECORD = PAGE_MAPPING as Record<string, { pageId?: string }>;

const DEFAULT_EXPERIMENT_PAGE_ID =
  PAGE_MAPPING_RECORD[String(EXPERIMENT_MIN_PAGE)]?.pageId || 'Page_01_Intro';
...
export function getInitialPage(subPageNum: string): string {
  ...
  return PAGE_MAPPING_RECORD[String(bounded)]?.pageId || DEFAULT_EXPERIMENT_PAGE_ID;
}
...
const EXPERIMENT_TASK_DURATION_SECONDS = 30 * 60;

export function getDefaultTimers(): { task: number } {
  return {
    task: EXPERIMENT_TASK_DURATION_SECONDS,
  };
}
