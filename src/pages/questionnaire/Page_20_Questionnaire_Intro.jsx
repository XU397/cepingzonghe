...
function Page_20_Questionnaire_Intro() {
  const { navigateToPage, startQuestionnaireTimer, setPageEnterTime, submitPageData } =
    useAppContext();

  // 数据记录Hook
  const { logButtonClick, logPageEnter, collectDirectAnswer } = useDataLogging(PAGE_ID);
  ...
  const handleStartQuestionnaire = async () => {
    ...
    try {
      ...
      if (!success) {
        console.warn('[Page_20_Questionnaire_Intro] P20提交失败，继续执行跳转到问卷第一页');
      }
    } catch (error) {
      console.error('[Page_20_Questionnaire_Intro] 提交P20数据失败，继续执行跳转:', error);
    }
    ...
    try {
      ...
      const navResult = await navigateToPage('Page_21_Curiosity_Questions', { skipSubmit: true });
      if (navResult === false) {
        throw new Error('导航被Flow拦截');
      }
    } catch (error) {
      console.error('启动问卷或跳转问卷首页失败:', error);
      alert('无法开始问卷，请稍后重试。');
    } finally {
  ...
  return (
    <div className={`${styles.pageContainer} ${styles.flowIntroPage}`}>
      <div className={`${styles.questionnaireContent} ${styles.flowIntroContent}`}>
        ...
        <div className={styles.introText}>
          ...
          <p>
            <strong>感谢你的配合！</strong>
          </p>
        </div>

        {!canProceed && (
          <div className={styles.readingTimer}>
            请仔细阅读说明，<span className={styles.countdown}>{timeRemainingOnPage}</span>
            秒后可开始作答...
          </div>
        )}
